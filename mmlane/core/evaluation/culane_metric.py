...
def load_culane_img_data(path):
    ...
    with open(path, 'r') as data_file:
        img_data = data_file.readlines()    # List[str(x0 y0 x1 y1 ...), str(x0 y0 x1 y1 ...), ...]
        ...
        filepaths = [
            os.path.join(
                data_dir, line[1 if line[0] == '/' else 0:].rstrip().replace(
                    '.jpg', '.lines.txt').split()[0]) for line in file_list.readlines()
        ]   # List[txt_path0, txt_path1, ...]

    data = []
    for path in filepaths:
        img_data = load_culane_img_data(path)   # List[[(x0, y0), (x1, y1), ...], [(x0, y0), (x1, y1), ...], ...]
