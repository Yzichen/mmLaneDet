...
import mmlane.core.evaluation.culane_metric as culane_metric
import mmcv
...
class CuLaneDataset(BaseDataset):
    def __init__(self, data_root, anno_files, pipeline, img_prefix='', seg_prefix='', test_mode=False):
        ...
        super(CuLaneDataset, self).__init__(data_root, anno_files, pipeline, img_prefix, seg_prefix, test_mode)
        self.h_samples = list(range(270, 590, 8))
        self.img_w, self.img_h = 1640, 590

    def load_annotations(self):
        os.makedirs('cache', exist_ok=True)
        cache_path = 'cache/culane_{}.pkl'.format('train' if not self.test_mode else 'test')
    ...
    def results2json(self, predictions, outfile_prefix):
        ...
        for idx, pred in enumerate(predictions):
            if isinstance(pred, list):  # List[lane0, lane1, ...]
                ...
                if len(pred) > 0 and pred[0].metadata.get('anchor', None) is not None:
