...
class non_bottleneck_1d(nn.Module):
    def __init__(self, channel, dropout):
        ...
        self.conv3x1_1 = nn.Conv2d(
            channel, channel, (3, 1), stride=1, padding=(1, 0), bias=True)
        self.conv1x3_1 = nn.Conv2d(
            channel, channel, (1, 3), stride=1, padding=(0, 1), bias=True)
        self.bn1 = nn.BatchNorm2d(channel, eps=1e-03)

        self.conv3x1_2 = nn.Conv2d(channel, channel, (3, 1), stride=1, padding=(1, 0), bias=True)
        self.conv1x3_2 = nn.Conv2d(channel, channel, (1, 3), stride=1, padding=(0, 1), bias=True)
        self.bn2 = nn.BatchNorm2d(channel, eps=1e-03)
...
class UpsamplerBlock(nn.Module):
    def __init__(self, in_channel, out_channel, up_height, up_width):
        super().__init__()
        ...
        self.follows.append(non_bottleneck_1d(out_channel, dropout=0.0))
        self.follows.append(non_bottleneck_1d(out_channel, dropout=0.0))

        # interpolate
        self.up_height = up_height
        self.up_width = up_width
        self.interpolate_conv = nn.Conv2d(in_channel, out_channel, kernel_size=1, stride=1, bias=False)
        self.interpolate_bn = nn.BatchNorm2d(out_channel, eps=1e-3)
...
class RESAHead(BaseDenseHead):
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(RESAHead, self).__init__(init_cfg)
        ...
        self.busd = nn.Sequential(
            UpsamplerBlock(in_channel=128, out_channel=64,
                           up_height=self.img_h // 4,
                           up_width=self.img_w // 4),
            UpsamplerBlock(in_channel=64, out_channel=32,
                           up_height=self.img_h // 2,
                           up_width=self.img_w // 2),
            UpsamplerBlock(in_channel=32, out_channel=16,
                           up_height=self.img_h,
                           up_width=self.img_w)
        )
        self.seg_conv = nn.Conv2d(16, self.seg_num_classes, kernel_size=1)

        self.bg_cls_weight = 0
        self.sync_cls_avg_factor = sync_cls_avg_factor
        class_weight = loss_seg.get('class_weight', None)
        if class_weight is not None and (self.__class__ is RESAHead):
